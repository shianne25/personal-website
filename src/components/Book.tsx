    ...
    return (
        <div className="cursor-pointer bg-[#c0c0c0] border-[3px] border-t-white border-l-white border-b-[#808080] border-r-[#808080] p-4 hover:brightness-105 active:border-inset group transition-all">
            <div className="flex justify-between items-start mb-2">
                <span className="text-[10px] bg-[#000080] text-white px-2 py-0.5 uppercase font-bold tracking-tighter">
                    {category}
                </span>
                <div className="flex text-[#ffcc00] drop-shadow-[1px_1px_0px_#000]">
                    {"★".repeat(rating)}{"☆".repeat(5 - rating)}
                </div>
            </div>
            <h3 className="text-lg font-bold text-black group-hover:underline decoration-2">{title}</h3>
            <p className="text-xs text-[#404040] mb-2">BY: {author.toUpperCase()}</p>
            <div className="border-t border-[#808080] pt-2">
                <p className="text-[11px] leading-tight text-black line-clamp-2 italic">"{commentary}"</p>
                <p className="text-[9px] mt-2 font-bold text-[#000080] text-right">[READ FULL ENTRY]</p>
            </div>
        </div>
    )
}